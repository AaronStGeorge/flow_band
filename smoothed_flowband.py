...
# Use binary search to find grounding line:
...
def find_gl(H,h_b):
  ...
  smin = 0.
  x = L/2.
  test = rho*H(x) + rho_w*h_b(x)
  while (abs(test) < GL_TOL):
    if test > 0:
      smin = x
      x = (smax - x) / 2.
    else:
...
if 'floating' in command_line_arguments:
  ...
  basal_drag = smooth_step * basal_drag
...
for i in range(nSteps):
  ...
  # Plot output from Kees's FORTRAN program Flow3.FOR
